...
// ReadByID implements core.IUserRepositoryInterface.
func (u *userRepositorySQL) ReadByID(ctx context.Context, lineUserID string) (user core.IUser, err error) {
	var query string = `
...
`
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			err = &core.ErrNotFound{OriginalError: fmt.Errorf("error at userRepositorySQL.ReadByID: %v", err)}
			return
		}
...
// Create implements core.IUserRepositoryInterface.
func (u *userRepositorySQL) Create(ctx context.Context, user core.IUser) (err error) {
	query := `
	...
	if err != nil {
		...
		if errors.As(err, &mysqlError) {
			...
			case 1062:
				err = &core.ErrDuplicate{OriginalError: fmt.Errorf("error at userRepositorySQL.Create: %v", err)}
			}
			return
		}
...
// Update implements core.IUserRepositoryInterface.
func (u *userRepositorySQL) Update(ctx context.Context, user core.IUser) (err error) {
	query := `
	...
	if errors.Is(err, sql.ErrNoRows) {
		err = &core.ErrNotFound{OriginalError: fmt.Errorf("error at userRepositorySQL.Update: %v", err)}
	}

	return
}

// DeleteByID implements core.IUserRepositoryInterface.
...
func NewUserRepositorySQL(db *sql.DB) core.IUserRepository {
