...
func (p *petRepositorySQL) ReadByID(ctx context.Context, id string) (pet core.Pet, err error) {
	var query string = `
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			err = core.NewErrPetNotFound(fmt.Errorf("error at petRepositorySQL.ReadByID: %v", err))
			return
		}
...
func (p *petRepositorySQL) ReadByUserID(ctx context.Context, userID string) (pets []core.Pet, err error) {
	query := `
	...
	if len(pets) == 0 {
		err = core.NewErrPetNotFound(sql.ErrNoRows)
	}

	return
}

func (p *petRepositorySQL) Create(ctx context.Context, pet core.Pet) (err error) {
	query := `
	...
	if err != nil {
		...
		if errors.As(err, &e); e.Number == 1062 {
			err = core.NewErrPetDuplicate(fmt.Errorf("error at petRepositorySQL.Create: %v", err))
			return
		}
...
func (p *petRepositorySQL) Update(ctx context.Context, pet core.Pet) (err error) {
	query := `
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			err = core.NewErrPetNotFound(fmt.Errorf("error at petRepositorySQL.Update: %v", err))
			return
		}
